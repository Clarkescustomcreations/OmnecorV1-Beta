/**
 * Third-Party Integration metadata — display labels, icons, scopes, and the
 * feature-capability map consumed by the integration UIs.
 *
 * NOTE: Actual OAuth account-linking, encrypted token storage, and live sync are
 * handled SERVER-SIDE via the tRPC `integrations.*` / `oauth.*` / `platforms.*`
 * routers. This module is purely the client-side presentation metadata used by
 * IntegrationsHub, ChatIntegrationBar, and MapManager. (Earlier client-side mock
 * token/connection helpers were removed once the real server backend superseded
 * them — see Progress-Tracker "Ruthless Public-Beta Code-Sweep".)
 */
...
export const INTEGRATION_FEATURES: Record<IntegrationType, string[]> = {
  ...
  notion:       ["chat"],
  slack:        ["chat"],
  "google-drive": ["chat", "neural-map"],
  dropbox:      ["chat", "neural-map"],
  onedrive:     ["chat", "neural-map"],
  outlook:      ["chat", "neural-map", "agent-networking"],
  gmail:        ["chat", "neural-map", "agent-networking"],
  generic:      ["chat"],
};

/** Display metadata for an integration type (title, description, icon, brand color, scopes). */
export function getIntegrationInfo(type: IntegrationType) {
  ...
  > = {
    ...
    "google-drive": {
      title: "Google Drive",
      description: "Connect Google Drive to browse files in the Neural Map",
      icon: "☁️",
      color: "bg-primary",
      scope: ["drive.file"],
    },
    dropbox: {
      title: "Dropbox",
      description: "Connect Dropbox to browse files in the Neural Map",
      icon: "📦",
      color: "bg-primary",
    ...
    onedrive: {
      title: "OneDrive",
      description: "Connect OneDrive to browse files in the Neural Map",
      icon: "☁️",
      color: "bg-primary",
