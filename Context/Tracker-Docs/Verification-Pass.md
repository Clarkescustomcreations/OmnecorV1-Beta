...
> **Path note (2026-07-12):** `server/phase2/` was renamed to `server/core_services/`. In the historical session narratives below, any `server/phase2/services|websocket|python_scripts/…` path now lives under `server/core_services/…` (filenames unchanged); `server/phase2/routers/…` and `LlamaCppService.ts` refer to files that were **removed** (duplicate routers deleted / the llama.cpp bridge retired), kept as an as-written record.

> Last updated: 2026-07-15 (Sessions 33–41) | Test suite baseline: **1741 tests passing, 4 skipped** (= 1745 total) across 154 test files (152 passed + 2 skipped files; measured 2026-07-15 via `pnpm test`, after the Brain Packs workstream Phases 1–8 + Generalist brain landed on top of the Blueprint Studio enhancements, the Session-37 fixable-gaps pass, Model-Fabric Phase 8, Chats-Agentic-Upgrade Phases 1–6, and Mesh-Delegation Phases 1–9). **Session-41 (2026-07-15) — Brains-Upgrade Phase 9 end-to-end live-proof, over OMMESH to Omnecor's OWN model hosting (NOT Ollama — per standing directive that the Ollama provider path skews eval):** two-node strict-mTLS mesh (`omnecor-linux` ↔ DadsPC `.201` `omnecor-dadspc`, mutual fingerprint approval). Coding brain imported live (`omnecor-coding`, status `ready`, 53 chunks, embedder-match ✓). `ai.chat` driven with `providerId:"llamacpp"` + `targetNodeId:"omnecor-dadspc"` + `brainIds:["omnecor-coding"]` against **qwen2.5-coder:7b on .201's own `llama-server` runtime** — proven-remote (this box has no local runtime; a pinned `targetNodeId` throws on offload failure instead of falling back, so a returned answer *must* have executed on .201; `.201` `/health` `localLlm:true`). **Grounded:** with-brain output returned the exact curated dynamic-SQL-identifier guidance ("Allow-List for Any Dynamic Identifier … reject input not in the allow-list") where baseline was muddled/wrong. **Cited:** model emitted real source tags `[Brain: Coding · sec-sql-dynamic-identifiers-allowlist]` + `[Brain: Coding · sec-sql-injection-parameterized]`. **Impossible→success:** Omnecor Expert brain (`omnecor-expert`, ready, 32 chunks) on an Omnecor-internal question — baseline hallucinated ("Omnecor is part of Alibaba Cloud", invented "Standard/Enterprise" modes), with-brain answered correctly (Sovereign/Scrapper/Big-Spender + `cloudProcedure` as the Sovereign-enforcing tier), cited `[Omnecor Expert · security-execution-modes]`. This also upgrades **real cross-node mTLS OMMESH inference via the `llamacpp` own-runtime** from 🤝 HARDWARE to ✅ live-verified (Linux→.201), and the `llama.cpp`/own-runtime inference path (§8 Priority 4) to ✅. **Gates:** `pnpm check` ✅ 0 errors · `pnpm test` **1741 passed / 4 skipped (152 files)** ✅. **Session-40 (2026-07-13) — Blueprint Studio enhancements (7 additions, 3 waves, all live-gated):** deepened the engineering coverage, closed the cut→BOM→shopping loop, and added existing-geometry import. **(1) Six new deterministic `engineering_calc` types** (each with formula workings + SF): `fillet_weld` (0.6·FEXX throat capacity), `bolted_connection` (governs bolt-shear/plate-bearing/edge-tear-out), `torsion` (round shaft T·c/J + angle of twist), `wood_joinery` (NDS lag/wood-screw withdrawal; end-grain refused), `printed_part` (FDM strength on the effective walls+infill section, layer-adhesion knockdown), `heat_check` ("will this plastic survive the sun / a hot car" — service-temp vs. a documented scenario/solar-gain peak). **(2) `optimize_cuts` `writeToBom`** upserts the computed buy-quantity onto the matching BOM stock line (by materialKey, else name) + saves the nesting run for provenance. **(3) Part revision history** (migration `0017`: `blueprint_files.version/supersedesId/isLatest`) — recompiling/re-importing a part supersedes its prior file; Drawings/3D/PDF read latest, Files tab shows version + dims superseded rows; persistence extracted to shared `persistPlanFile` (fileStore.ts). **(4) Async FEA** — `run_fea` writes a `running` row up-front and solves **decoupled from the chat-stream signal** (a disconnect no longer kills a multi-minute solve; a background handler updates the row), Simulation tab shows a RUNNING badge → completed. **(5) Shopping export** — `blueprint.exportBom` → RFC-4180 CSV + supplier-grouped printable buy-list + known-price rollup (Sovereign-safe, no network); BOM-tab "Export list" button. **(6) Geometry import** — `blueprint.importGeometry`: STL (reuses `parseStl` → a full part: 3D + drawings + FEA-ready) or DXF (minimal LINE/LWPOLYLINE reader → outline-preview SVG + original DXF); STEP/IGES deferred (no heavy CAD kernel); "Import STL/DXF" control on the 3D tab. **+30 tests (1560 → 1590)**, 2 new files: `blueprintAgentStream.test.ts` (3 — subscription turn-persistence + empty-history filtering, mocked runner/CAD/FEA), `blueprintPlanPdf.test.ts` (3 — booklet structure + WinAnsi transliteration); extended `blueprintCalcEngine.test.ts` (23→37), `blueprintCadService.test.ts` (20→23 DXF reader), `blueprintRouter.test.ts` (13→20 cut→BOM/revision/async-FEA/export/import). **`/review` hardening (6 fixes):** `heat_check`/`printed_part` now reject out-of-enum scenario/mode with clear errors (were a `peakC` crash / silent bending); `requiredSafetyFactor` throws on a bad basis (was silent `undefined`→always-fail); `persistPlanFile` clears-old-latest + inserts atomically via `db.batch` (crash-safe; interactive txn unusable on in-memory libSQL); dropped an unused `printedPart` E param; `opacity-55`→`opacity-60` (not a real Tailwind step). **Gates:** `pnpm check` ✅ · `pnpm test` **1590 passed / 4 skipped** ✅. **Session-39 (2026-07-13) — Blueprint Studio (AI-assisted fabrication planning), new top-level page + agentic toolset, live-verified end-to-end:** a new `/blueprint-studio` page + `blueprint.*` router turn a plain-language physical-project brief (carpentry / metal fab / structures / vehicles / 3D printing / multi-part costumes) into a persistent **Build Plan** — BOM, cut list (with miter/bevel angles), dimensioned drawings, compiled 3D geometry, true-scale patterns, deterministic engineering verification, and assembly steps — attached to the active Neural Map and exportable as a PDF booklet. **Reusable harness extension:** `ChatAgentRunner` now takes `extraTools: ExtraAgentTool[]` + `includeBuiltInTools:false`, so a feature can run the agentic loop with a **pure domain toolset** (dispatched before MCP fallthrough, rendered as the existing `mcp` tool box, no HITL, no file/shell access) — the text tool-protocol prompt renders injected tools generically from their JSON Schema; proven built-in wording unchanged. The **11-tool Blueprint set**: materials-catalog search, deterministic `engineering_calc` (beam/column/fastener/rafter/stairs/compound-miter/triangle — the model is forbidden from doing structural math itself; results persisted with formula workings + safety factors), `optimize_cuts` (1D/2D/fabric nesting), plan writers (`update_plan`/`set_bom`/`set_cut_list`), `compile_cad`, `generate_pattern`, `run_fea`, `generate_concept_image`, and cloud-gated `search_materials_web` (**omitted entirely for sovereign users**). **Deterministic calc engine** (`calcEngine.ts`, pure-TS SI-metric, basis-aware SF: allowable 1.0 / yield 1.67 / ultimate 2.5 + FDM layer-adhesion knockdown). **Offline materials DB** (61 entries, 11 categories — NDS №2 lumber design values, ASTM A36/A500/6061/4130 metals, filament datasheets, fabrics/EVA/Worbla) keeps Sovereign mode fully functional. **Dual-engine CAD** (`BlueprintCadService`): JSCAD (`@jscad/modeling`) in a `node:vm` sandbox as the zero-install default; OpenSCAD as the optional external binary (`openscadPath` setting, `--version` probe — Blender/KiCad pattern) → viewer mesh JSON + binary STL + dimensioned three-view drawing SVG + R12 DXF. **Real FEA** (`fea_bridge.py` + `BlueprintFeaService`): Gmsh tet mesh + TET4 linear-static elasticity (numpy/scipy) with von-Mises heatmap; optional dep (`pip install gmsh numpy scipy`) probed with graceful degradation. **6 new tables** (`blueprint_plans/_bom_items/_cut_items/_files/_sim_results/_messages`), migration `0016`, all dims stored in mm. **4 real bugs fixed in passing:** (1) live-install **migration drift** — an older-generation `0015` re-ran, failed on an existing object, and the auto-migrator warn-and-continued, **silently skipping every later migration incl. `0016`** (repaired the drifted DB + `pnpm db:migrate`; `/health` `migrationOk:true`); (2) **Gemini streaming returned empty responses with no error** — `:streamGenerateContent` needs `alt=sse` or the per-line parser drops the pretty-printed JSON array (zero chunks); (3) **`exportPdf` crashed in prod builds** (`__dirname is not defined`) — pdfkit/svg-to-pdfkit marked esbuild externals + WinAnsi glyph transliteration; (4) **conversation self-poisoning** — empty assistant turns no longer persisted, empty-content history filtered before provider calls (Gemini 400s on empty parts). **+56 tests (1504 → 1560)**, 3 new files: `blueprintCalcEngine.test.ts` (23 golden-value), `blueprintCadService.test.ts` (20 compile/sandbox/STL/drawings/pattern + extraTools prompt), `blueprintRouter.test.ts` (13 route-level ownership + toolset against the real in-memory DB). **Gates:** `pnpm check` ✅ · `pnpm test` **1560 passed / 4 skipped** ✅. **Session-38 (2026-07-12) — `/review` + `/architect` pass on the Session-37 model-download work (live-verified, no net new tests):** `/review` caught a whole-page **crash class**: Chat + 3D Designer **statically imported** the heavy preview panels (`EnhancedPCBEditor`/`ThreeViewer`/`WebPreview`), so a throw in a preview's module graph failed the *page's own* module and dropped the entire route to RouteBoundary (this is what made the Chats / 3D-Designer pages fail to load). Fixed at the root: new `lazyWithRetry` (`client/src/lib/lazyWithRetry.ts`) + `LazyPreviewPane` (Suspense + per-pane `ErrorBoundary`) code-split and isolate every conditionally-rendered preview; an inert `EditorToolbar` import-alias that had masqueraded as the fix was reverted (a named-import rename can't fix a `ReferenceError`). Also silenced the dev-only `Uncaught (in promise)` / Vite-HMR console noise at the root — `OmnecorWebSocketServer` moved from `{ server, path:"/ws" }` (which aborts every non-`/ws` upgrade, killing the Vite HMR client socket) to **`noServer` + a manual `/ws`-only upgrade router** (`verifyClient`/origin/auth unchanged; mesh mTLS is a separate server). And renamed `modelMarketplace.downloadModel` input `filename`→`filePath` (it carries a repo-relative `.gguf` path, not a basename). **No net new tests** (updated `modelMarketplaceRouter.test.ts` for the rename). **Live-verified via chrome-devtools MCP:** Chat + 3D Designer render clean, the PCB/Web/3D chunks load via `lazyWithRetry`, the console error log is **empty**, and the app `/ws` still upgrades (`appWsProbe:"open"`). **Gates:** `pnpm check` ✅ · `modelMarketplaceRouter` tests ✅. **Session-37 (2026-07-11) — "fixable-now gaps" pass (no hardware):** **(1) MoE-Chain moved onto Omnecor's own runtime** — `MoeChainService` local steps now call new `AiProviderService.completeLocal()` (hot-swaps the managed `llama-server` per step via `ensureModelLoaded`; the swap frees the prior model's RAM), and the standalone `llamacpp_bridge.py` / `LlamaCppService` / the `:8013` health entry were **retired** (fully dead after the migration — `getEmbedding` was unused, nothing else called it). **(2) TD-042 (PCBWay) closed** — the quote/order path was already real since 2026-06-23 (parametric quote + real Gerber-ZIP multipart order); added the missing test coverage and corrected the stale "Open" header. **(3) Hugging Face model download** — new `ModelMarketplaceService.listRepoFiles` + streaming downloads: a single **GGUF quant** into `PATHS.models` (auto-indexed → runnable on the local runtime, surfaced as a new **"HF GGUF"** tab in the Model Hub) and a whole **base-model repo** (config + tokenizer + safetensors, skipping GGUF/other-framework blobs) into `PATHS.baseModels` for **offline/sovereign fine-tuning** in the LLM Builder (`UnslothPanel` "Browse HF" → sets the base model to the local path). Keyless HF search; not Sovereign-gated (a download is a fetch, not inference). Live-verified the real HF tree + resolve contract via curl. **`/code-review` + `/review` found and fixed:** the migrated MoE step passed `step.modelPath` (a **directory**) to the loader instead of `path.join(modelPath, ggufFile)` — a latent wrong-model bug (MoE-local was never hardware-verified, so it hid); the LLM Builder **Start Training** button didn't pass `modelName`, so the base-model field never reached the trainer (now wired); and `LocalLlmRuntimeService.ensureModelLoaded` now **throws for an unindexed model** rather than silently serving the loaded one (wrong-weights guard). Plus a HF free-space pre-flight guard, concurrent-download de-dup, and a capped in-memory download registry. **+35 tests (1469 → 1504)**, new files `pcbwayService.test.ts` (8), `kicadRouterPcbway.test.ts` (3), `modelMarketplaceService.test.ts` (10 — incl. GGUF/base-model streaming, dedup, free-space) + extensions (`moeChainService.test.ts` +2 incl. dir/file join, `modelMarketplaceRouter.test.ts` +6, `LocalLlmRuntimeService.test.ts` +1 unindexed-throw). **Gates:** `pnpm check` ✅ · `pnpm test` **1504 / 4-skip** ✅ · `pnpm build` ✅ · `pnpm audit --prod` ✅. **Session-36 (2026-07-10) — Model-Fabric Phase 8: local GGUF auto-discovery + hot-swap (Omnecor's own runtime hosts *every* local model, no manual adds):** new **`ModelIndexService`** auto-discovers every GGUF on the box — the app models dir **and the Ollama blob store** (parses `~/.ollama/models/manifests` → model-layer digest → `blobs/sha256-*`, reconstructs real names like `deepseek-r1:14b`, GGUF-magic gated, content-deduped by size+64 KB-header-hash) — read straight off disk so it works with **Ollama stopped** (Ollama truly optional). `LocalLlmRuntimeService` gained `ensureModelLoaded(idOrPath)` **hot-swap** (stop current `llama-server`, spawn the requested model), **per-model `--n-gpu-layers` VRAM fit** via `collectGpuTelemetry` (fits→all, else proportional partial offload — no OOM crash-loop for a 14B/27B on 8 GB), `isAvailable()`, `getLoadedModelId()`, and last-model persistence (`localLlmLastModel` → boot resumes it, else idle/load-on-select). `ModelCatalogService` now lists **all** indexed models as `omnecor-runtime` (warm one flagged `loaded`, ready-gated) and **skips the live Ollama API source when the runtime is available** (the index already covers the store — no double-listing). New non-blocking `aiProvider.loadLocalModel` mutation; `ModelSelector` pending-state "loading…→loaded" indicator (`bg-accent-success` token, not raw emerald). **`/review` found 6 issues, all fixed** (Important: serialized ALL lifecycle work — boot load, hot-swap, crash respawn — through one queue so nothing orphans a server on :8014; `_waitForHealth` bails on proc-death; async non-blocking index scan; ready-gated loaded flag; content-clean source-gating; CPU-safe GPU fallback). **Live-verified on DadsPC (.201, RTX 4060 Ti):** all 10 Ollama GGUFs render as **"Omnecor · This PC"** (0 Ollama-branded), boot-resume + hot-swap + inference (`REVIEW_FIX_OK`) + persistence all confirmed. **+22 tests (1447 → 1469)**, 1 new file (`ModelIndexService.test.ts` 8). **Gates:** `pnpm check` (root + APK) ✅ · `pnpm test` **1469 passed / 4 skipped** ✅. Still NOT done: ~~HF browse/download UI~~ (**done Session-37**); ~~MoE-Chain still on the separate `LlamaCppService`/:8013 bridge~~ (**migrated + bridge retired, Session-37**); APK picker loaded-indicator UI (type mirrored only) **remains**. **Session-35 (2026-07-08) — Mesh Sub-Agent Delegation (Phases 1–9):** built the full `delegate_task` tool loop on a trusted OMMESH peer — `SubAgentHostService` (sandbox, concurrency cap, kill-switch, isolated HITL broker, executionMode enforcement, cursor-replay buffer, grace-window abort, `AsyncJobService` continuation), 4 mTLS mesh endpoints (`POST /subagent`, `GET /subagent/:id/stream?since=N`, approval/cancel), `DelegationService` (peer NDJSON consume → persist turns → re-publish live → forward HITL + cancels → cursor re-attach → synthetic AsyncJob re-prompt), `delegate_task` tool (origin-only, always HITL-gated even under autoApprove), `delegationRouter` (`stream`/`sendTurn`/`cancel`/`status`), web + APK managed chats (Network badge, node name, live stream fold, between-turn input, cancel). 33 new tests. **Gates:** `pnpm check` (root + APK) ✅ · `pnpm test` **1447 passed / 4 skipped** · `pnpm build` ✅ · `pnpm audit --prod` ✅. **Session-34 (2026-07-07–08) — Model-Fabric: Ollama decoupling + unified model catalog (Phases 0–7):** `LocalLlmRuntimeService` (managed `llama-server` subprocess; Ollama now optional), dual `ToolProtocol` seam (text floor + native function-calling upgrade), unified `ModelCatalogService` (4-source aggregator: local runtime + optional Ollama + mesh peers + cloud; Sovereign gate added — Critical fix), beacon-minimal mesh advertising (`modelsHash` only in mDNS TXT; full list fetched over mTLS `GET /models`), `describeCatalogHost()` single source of truth (web + APK), `ModelSelector` rewrite, APK `listCatalogGroups()`, `targetNodeId` mesh-pin, per-node Omnecor Model Hub tab, live-hardware-verified (`llama-server` + real Llama-3.2-3B). 41+16+15+2+3 = 77+ new/changed tests across 10+ files. **Session-33 (2026-07-04–07) — Agentic Chat Stream — web + APK (Chats-Agentic-Upgrade Phases 1–6):** `AssistantBlock[]` typed stream (guide-line, thinking, command/edit/job/mcp chips), `ChatAgentRunner` (streaming typed-event tool loop — `edit_file`/`run_command`/`start_job`, HITL via `ToolApprovalRegistry`, `validatePath`-enforced, `MAX_TURNS=8`), `agentChatStream`/`resolveToolApproval`/`runCodeSnippet` tRPC surface, FIFO message queue (Zustand, not persisted), code block execution toolbar, auto-approve toggle, `wsAuthBridge` APK WS auth fix, full APK port (native block renderers, on-device GGUF/LiteRT fold, LCS line-diff). 122 new tests (1193 → 1315). **Session-32 (2026-07-04) — APK on-device model stack, live-verified on the physical S25 Ultra over adb (Snapdragon 8 Elite):** drove the whole on-device LLM path end-to-end and hardened it (moves `(tabs)/ai-node.tsx` LiteRT-LM + the two APK-checklist LiteRT items from 🤝 HARDWARE/☐ to ✅; details in Tech-Debt TD-006/051/052/053). **GGUF (llama.rn):** download→completeness-validate→load→chat verified (Llama-3.2-3B answered, PC offline); fixed the real "Load failed" — `use_mlock`+full-GPU-offload of a 4.7 GB model, plus a **truncated download shown as "✓ Downloaded"** (existence-only check) now size-validated → "⚠ Incomplete — re-download". **LiteRT-LM:** DeepSeek-R1-1.5B **and Gemma-4-E4B** loaded + answered on-device (Gemma-4 on the **GPU** backend, no crash) after fixing a hard APK crash — loading segfaulted Hermes (`mqt_v_js`, pure `libhermesvm.so` stack) via our `npu→gpu→cpu` **close-and-recreate loop** (use-after-free on the Nitro engine) → now **one engine / one `loadModel`**, CPU default, opt-in GPU/NPU toggle. **Model acquisition built + verified:** ungated `.litertlm` download catalog, SAF **folder-scan** (Android blocks `Android/data/` and Downloads/root SAF grants → must grant a sub-folder like `Download/OmnecorModels`), single-file import, and a chat **"📱 Phone (on-device)" provider** (grouped, re-scans on focus) so any on-device GGUF/`.litertlm` is selectable and runs with no PC. Also fixed in passing: the chats-page fake always-green "Connected" dot (now real health-probe state). **Not yet proven:** GPU/NPU across *all* models (only Gemma-4 GPU + CPU proven — TD-051); true NPU via AICore/Gemini Nano (TD-052).
 **Session-31 (2026-07-02) — second pass + doc-accuracy audit:** re-ran all gates (`pnpm check` 0 errors; suite re-measured green at the 1134 baseline before changes), then (a) **audited every doc row against the actual routers** and removed/corrected **6 phantom procedures that do not exist in the code** (`security.getThreatReport`, `security.getAuditSummary`, `job.getResult` (it's `getStatus`), `discovery.refreshFeed`, `ommesh.revokePeer`, `ommesh.getTopology`) plus stale 🧪/🔬 statuses left behind after Batch G landed (job/discovery/ommesh/workflow/penpot/system sections now reflect their real test files); and (b) **converted the remaining no-dependency 🧪/🤖 rows to ✅ with +60 new tests across 8 files** (2 new files): `comfyRouterMock.test.ts` (11 — ComfyService-mocked delegation + bridge-offline INTERNAL_SERVER_ERROR mapping for queuePrompt/getQueue/getSystemStats/interrupt/clearQueue), `pairingRouter.test.ts` (8 — route-level createCode QR payload + device-role FORBIDDEN, listDevices per-user ordering, revokeDevice ownership + in-memory revocation), `securityRouter.test.ts` 12→29 (scanFile/scanDirectory/encryptFile/decryptFile/generateProjectKey/createBackup/restoreBackup/listBackups/runVulnerabilityScan — real `validatePath` gate + delegation + threat-aggregation math), `platformsRouter.test.ts` 9→14 (getPublishingRouting webhook config incl. sovereign+remote-n8n blocked flag; setWebhookPath admin gate + slash-trim + default-restore, settings isolated via `__testSettingsPath`), `virtualCardRouter.test.ts` 11→14 (revealCardPan sovereign gate / delegation / null→NOT_FOUND), `integrationsRouter.test.ts` 14→17 (updateSettings sovereign gate, not-connected NOT_FOUND, settings persisted + metadata merge), `projectRouter.test.ts` 10→16 (**real-fs happy paths** under `PATHS.projects`: writeFile→readFile round-trip, missing-file BAD_REQUEST, getFileTree children/non-dir/traversal, registerProject resolved-path handoff), `ollamaRouter.test.ts` 6→13 (listModels/runningModels/modelInfo/createModelfile against a mocked daemon: payload mapping, `Ollama error: <status>` mapping, empty-safe). **Session-30 (2026-07-02):** the user reported the Chats page "Terminal/CLI" (EmbeddedTerminal.tsx — the live xterm.js + node-pty shell, distinct from the Docker "Sandboxed" terminal) couldn't run commands at all. Root cause: the client (`EmbeddedTerminal.tsx`) and server (`WebSocketServer.ts`) had **independently duplicated, silently-drifted** `pty:*` WS message shapes — every keystroke was sent as a bare string where the server expected `{input: string}`, so `message.data.input` was always `undefined` and **no input ever reached the shell**. Fixed by extracting the shared contract into `shared/types/terminal.types.ts`, imported by both sides (drift is now a compile error, not a silent runtime bug), plus fixing `pty:output`/`pty:exit` parsing and adding a previously-unhandled `error` message case. Also found and fixed, in passing: `server/phase2/services/MeshDiscoveryService.ts` (a legacy duplicate mDNS advertiser) had no `error` listener on its bonjour service, so any node-name collision on the LAN (trivially reproduced by running two Omnecor instances) threw an unhandled `error` event and **crashed the entire server process** — mirrored the existing fix already present in `server/ommesh/core/DiscoveryService.ts` for the identical bonjour quirk. Then built the previously-unwired **"AI can push a command into your terminal"** feature end-to-end: system prompt discloses a `<terminal_command>` directive only when the terminal is open (non-fiction-mode only); `client/src/lib/terminalDirective.ts` extracts it from a completed assistant message and strips it from the displayed/persisted text; `omnecor:cli_command` CustomEvent dispatch; `EmbeddedTerminal.tsx` gates it through the **same** `requestApproval()` HITL flow as manual typing (AI-initiated commands are never trusted more than typed ones) before writing to the PTY, auto-opening the terminal via a new `onRequestOpen` prop if it wasn't already visible. The insecure, dead `chat:toTerminal` WS message type (bypassed HITL entirely, never sent by any code path) was removed. All of this was **live-verified via chrome-devtools MCP** against the real dev server: manual typing (`echo` round-trip through a real bash PTY with the user's actual shell MOTD), HITL approval/denial, and the AI-initiated bridge (dispatched with the terminal closed → HITL dialog fired → approved → terminal auto-opened → command executed with real output) all confirmed working. Added `client/src/lib/terminalDirective.test.ts` (5 tests) for the new parser; a full WS+PTY integration test was assessed but not added — `OmnecorWebSocketServer`'s constructor calls the real `createContext()`/`getDb()` (not the isolated in-memory `createTestDb()` harness other router tests use) plus 7 singleton services, so it would touch the real `~/.omnecor/data/omnecor.db` — unsafe without harness work first (see Section 12 backlog). **ESP32 hardware fully closed 2026-06-30:** `esp.compile` (arduino-cli) + `esp.flash` were driven through the real tRPC router against a physical ESP32-D0WD-V3 — merged image flashed at `0x0`, board rebooted and BLE-advertised `OMNECOR_TEST_OK` (confirmed over serial). Fixed a real flash bug in passing: `esptool_bridge.py` hardcoded offset `0x1000` (bootloader slot) which left any app image unbootable — offset + chip are now plumbed params (default `0x0`). This includes the **Batch G** harness-driven router tests — **complete** (the final 4 routers `pcbEditor`/`podcast`/`aiProvider`/`integrations` landed this pass) — and **Batch D** DB-schema tests (`dbSchema.test.ts`), which surfaced and fixed a **real cascade bug**: 6 child tables of `neural_maps` had NO-ACTION FKs in the actual DB (added via `ALTER … ADD COLUMN … REFERENCES`, which silently drops the action) despite `schema.ts` declaring `onDelete: cascade`, so deleting a non-empty project/map threw a FK error and orphaned its rows. Fixed by migration **0014** (rebuilds the 6 tables with real `ON DELETE cascade`) **plus** an atomic app-level `db.batch` cascade in `neuralMaps.delete` (belt + suspenders). Also the social-publishing changes: `PublishingService` rewritten to a hybrid (n8n webhook for X/LinkedIn/Facebook/Instagram + native Bluesky/Mastodon/Discord/Telegram), new `WebhookPublisher`, a `publishNow` IDOR fix and `addAccount` idempotency. Then **Batch E** (AI harness paths) landed `valetRouter.test.ts` (route/getMoeChain/saveMoeChain/initMoeChain/scanLocalModels against a stubbed classifier + mocked GGUF scan) and the `ai.chat` **map-RAG-injection** path (augmented messages forwarded to the provider). **Batch F (Agentic Wallet / Lithic)** landed `virtualCardRouter.test.ts` (ownership + **PAN-safety** on getCard/listCards, issueCard sovereign gate + not-configured + HITL approve/deny + 1/60s rate limit) and an **in-suite Lithic mock** for `VirtualCardService.listTransactions` (env-switched `LITHIC_API_BASE` → fake host, response mapping, ownership/error paths — no money). Batch F also landed `oauthRouter.test.ts` — the OAuth code flow against the **real DB + real PKCE state store**: getAuthorizationUrl persists state+verifier and sends a matching SHA-256 **PKCE challenge**, handleCallback validates state (bogus/cross-user/TTL → BAD_REQUEST) and persists the account, disconnectAccount enforces ownership; in passing, fixed a catch-all that masked inner `TRPCError` codes (UNAUTHORIZED/BAD_REQUEST) in both procedures. **Batch I (Python-bridge routers)** landed `voiceRouter.test.ts` (Whisper/TTS/RVC health aggregation + **bridge-offline degradation**: unreachable→PRECONDITION_FAILED, missing→NOT_FOUND, bad-wav→BAD_REQUEST; ElevenLabs cloud sovereign gate) and `agentRouter.test.ts` (crew/liteAgent/n8n delegation + **RecursiveMAS HITL gate** for >3-agent crews + stop-via-processManager). The 4 bridge-gated suites — `comfyRouter` (1), `blenderRouter` (3), `kicadRouter` (3), `espRouter` (6) — auto-skip individually when their tool is absent (only `comfyRouter` skipped on this run; 1120 passing / 0 skipped with ComfyUI up). **Batch F tail closed 2026-06-30:** `gmailRouter.test.ts` (17) drives `gmail.status` + `gmail.sendEmail` end-to-end against the real DB + a stubbed `fetch`/`oauthClients` — config/connection guards, the Bearer token + endpoint + RFC-2822 payload actually sent, **refresh-on-401 token rotation persisted to the DB**, API-error mapping, per-user ownership isolation, and the Sovereign gate. **Batch G tail closed 2026-07-01:** a router-coverage audit (every namespace in `routers.ts` vs. its test file) found **6 registered routers with no dedicated route-level test** — now added: `discoveryRouter` (8, article-feed filters + ingest), `jobRouter` (14, HITL command gate + cwd validatePath + admin gates), `workflowRouter` (8, the review/remember/imprint skill-commands — real git diff, secret-redacted memory.md write, `validatePath` traversal reject), `penpotRouter` (6, identifier-regex filename guard), `ommeshRouter` (13, mesh control + admin gates + crossNodeSync/agentDiscourse persist→read-back) and `systemRouter` (16, health/config-status booleans-not-secrets + settings round-trip incl. the admin-only `sovereignBlockAiOnly` guard + RBAC + DB user-admin). Every registered router now has route-level coverage — bringing the suite to 1119 passing across 101 files. **Session-29 (2026-07-01) — live-driven Batch H/I + 2 real bug fixes:** drove the authenticated web UI headless (13 feature pages render + fire their real backend tRPC queries; chat loop end-to-end incl. a **live Ollama token stream** on the LAN server) and stood up the **real voice bridges** — `voiceBridges.test.ts` transcribes an espeak clip via `whisper_server.py` (faster-whisper) and synthesizes speech via `tts_server.py` (XTTS-v2), both through the real router (auto-skip pattern), which required fixing 4 missing `requirements.txt` deps (python-multipart, torchaudio, transformers `>=4.57,<5`, torchcodec). Fixed **two real bugs found live**: **TD-047** — any AI stream timeout/disconnect crashed the whole server (`ERR_INVALID_STATE: Controller is already closed`), now guarded via `guardedEmit` across all 3 `chatStream` subscriptions (+6 unit tests); and the Valet **5s route timeout** (too short for local/thinking models → constant fallback), now `VALET_ROUTE_TIMEOUT_MS` (60s) + bridge `VALET_OLLAMA_TIMEOUT` (120s). Suite now **1128 passing + 1 skipped = 1129 across 103 files** (voice + valet bridges live; comfyRouter skipped without ComfyUI).
>
...
## Section 1 — Current Automated Test Suite (1590 passing, 4 skipped, 140 files)

All tests run with `pnpm test`. This section is the ground truth for what is actually automated today. **Authoritative live `pnpm test` (2026-07-10, after Model-Fabric Phase 8 landed) = 1469 passing / 4 skipped.** Net new tests across the four workstreams: **276** (1193 → 1469). Key test files added or substantially extended: `chatBlocks.test.ts` (8), `agentStream.test.ts` (12), `app.store.test.ts` (11), `ChatAgentRunner.test.ts` (+28 cumulative), `aiProviderRouter.test.ts` (+8 cumulative), `wsAuthBridge.test.ts` (7), **`ModelIndexService.test.ts` (8 — Phase 8, new file: models-dir + Ollama-store scan, name reconstruction, GGUF-magic gate, content-dedup, non-blocking cache read)**, `LocalLlmRuntimeService.test.ts` (14 — rewritten for Phase 8 hot-swap: `ensureModelLoaded`, per-model VRAM-fit `_computeGpuLayers`, serialized boot/swap/crash, persisted-model boot-resume), `AiProviderService.streamChat.test.ts` (25 — llamacpp now hot-swaps via `ensureModelLoaded`), `toolSchemas.test.ts` (new), `ModelCatalogService.test.ts` (24 — lists all indexed as `omnecor-runtime` + ready-gated `loaded` flag + Ollama source-gating), `aiProviderRouterCatalog.test.ts` (6 — Sovereign flag passthrough + non-blocking `loadLocalModel`), `discoveryService.test.ts` (+8 beacon/hash), `meshServerModels.test.ts` (5 — first-ever `MeshServer.handleRequest` tests), `meshNodeModelCatalog.test.ts` (5 — first-ever `MeshNode` tests), `tokenCrypto.test.ts` (+`hashModelList` cases), `AiProviderService.selectPeerNode.test.ts` (4), `SubAgentHostService.test.ts` (18), `DelegationService.test.ts` (5), `meshServerSubAgent.test.ts` (5).

### 1.1 Server — Router & Core Tests (103 files, ~1006 tests)
...
| `server/__tests__/aiProviderRouter.test.ts` | cumulative | **Chats-Agentic-Upgrade + Model-Fabric.** `agentChatStream` Sovereign gate (protectedProcedure + per-provider `assertProviderAllowedInMode`); `resolveToolApproval` ownership check; `targetNodeId` survives `agentChatStream`'s manual field reconstruction; `aiProvider.catalog` Sovereign/scrapper/big_spender flag passthrough + **Phase 8 `aiProvider.loadLocalModel`** (non-blocking hot-swap trigger — asks the runtime to load + returns `{started}` immediately, never rejects on a background load failure, empty-modelId schema reject) (`aiProviderRouterCatalog.test.ts` 6) |
| `server/__tests__/wsAuthBridge.test.ts` | 7 | **Chats-Agentic-Upgrade.** `wsAuthBridge`: promotes APK `?token=` query param to `Authorization: Bearer` before tRPC WS handler; cookie/Bearer callers untouched; covers the silent APK WS auth failure fixed in production |
| `server/__tests__/delegationRouter.test.ts` | — | **Mesh-Delegation.** `delegation` router (`stream`/`sendTurn`/`cancel`/`status`); `resolveToolApproval` transparent peer-forward for delegated approval IDs; `delegationEvent` WS lifecycle broadcast (started/turn-complete/done/failed) |
| `server/__tests__/SubAgentHostService.test.ts` | 18 | **Mesh-Delegation.** Run registry, sandbox `validatePath`-enforced, concurrency cap + kill-switch, isolated `ToolApprovalRegistry`, `executionMode` enforcement, cursor-replay buffer, grace-window abort, `AsyncJobService` continuation |
| `server/__tests__/DelegationService.test.ts` | 5 | **Mesh-Delegation.** Peer NDJSON consume, persist turns, HITL forward, cursor re-attach, parent re-prompt via synthetic AsyncJob |
| `server/__tests__/meshServerSubAgent.test.ts` | 5 | **Mesh-Delegation.** First-ever `MeshServer.handleRequest` subagent tests: `POST /subagent` spawn/follow-up, `GET /subagent/:id/stream?since=N`, approval/cancel — all behind pinned-peer trust gate |
| `server/__tests__/meshServerModels.test.ts` | 5 | **Model-Fabric.** First-ever `MeshServer.handleRequest` tests for the `/models` route: trust gate (403 untrusted), returns `{models: NodeCapabilities.models[]}` for trusted peer; `/health` stays pre-trust reachable |
| `server/__tests__/meshNodeModelCatalog.test.ts` | 5 | **Model-Fabric.** First-ever `MeshNode` tests: catalog-to-`NodeCapabilities.models` mapping, catalog-failure fail-safe, hash/GPU re-advertise gating (`SecurityManager`/`DiscoveryService`/`RoutingEngine`/`MeshServer`/`HostTelemetry` mocked) |
| `server/__tests__/ChatAgentRunner.test.ts` | cumulative +28 | **Chats-Agentic-Upgrade + Mesh-Delegation.** Prose split, `edit_file` approve/deny/no-root, `run_command`, `start_job`+denial, MCP nested-arg parse, diff/JSON helpers; native decode/execute; stray-tag safety net; malformed-native-args retry; `parseToolCallText` closed/unclosed/no-JSON/no-tag; `delegate_task` always-HITL gate (+4) |
| `server/__tests__/blueprintRouter.test.ts` | 20 | **Session-39/40 (Blueprint Studio).** Route-level against the real in-memory libSQL harness (`createCaller`, FK cascade on): plan CRUD + status/overview update; **per-user ownership isolation across every plan-scoped endpoint** (get/update/delete/listMessages/upsertBomItem/exportBom/importGeometry → TRPCError for a non-owner); mapId filter; child-row **cascade** (BOM/cut/messages); BOM/cut-list edit fidelity; conversation persistence; materials surface; **agent toolset against the same DB** — `update_plan`/`set_bom`/`set_cut_list`/`engineering_calc` persist + snapshot, non-owner refused, `search_materials_web` sovereign-omitted, `list_materials` keys usable. **Session-40:** `optimize_cuts` **writeToBom** upsert + provenance sim-row; **compile_cad revision lineage** (v1 superseded → v2 `isLatest`, `supersedesId` back-link); **async `run_fea`** (mocked service) — a `running` row exists mid-solve, `fea.run` called with the request ONLY (no signal → survives disconnect), then row → completed + `fea_result` file; **`exportBom`** CSV RFC-4180 escaping + supplier grouping + known-price total; **`importGeometry`** STL→full part (mesh/stl/drawing_svg/drawing_dxf, `imported` flag) + DXF→outline drawing + empty-DXF/non-owner rejects |
| `server/__tests__/blueprintAgentStream.test.ts` | 3 | **Session-40 (Blueprint Studio).** Subscription-level `blueprint.agentStream` (mocked `ChatAgentRunner` + stubbed CAD/FEA probes): user turn persisted before streaming + non-empty assistant turn persisted on `done` with `blocks`/`tokenCount`; **empty-content history filtered** out of the messages handed to the runner; non-empty-turn gate (whitespace + zero blocks → not persisted); non-owner → error, runner never constructed, nothing written |
| `server/__tests__/blueprintPlanPdf.test.ts` | 3 | **Session-40 (Blueprint Studio).** `buildPlanPdf` booklet: valid `%PDF-`…`%%EOF` with an embedded drawing SVG; **WinAnsi transliteration** of Greek/math glyphs (σ τ δ ⁴ ≥ ✓) in calc workings without throwing; empty-plan (no BOM/cut/sim/drawings) still renders |

### 1.2 Server — Service Tests (19 files, ~270 tests)
...
| `…/services/__tests__/ModelIndexService.test.ts` | 8 | **Model-Fabric Phase 8 (new).** Local GGUF discovery over a virtual `fs/promises`: scans `PATHS.models` (skips `valet-router/`, GGUF-magic gate rejects non-GGUF `.gguf`), indexes the **Ollama blob store** with real names reconstructed from manifests (`registry.ollama.ai/library/` stripped; `hf.co/…` preserved), **content-dedup** collapses a models-dir hardlink + its Ollama blob (size + 64 KB header hash), `resolve()` by id/path/name, and `list()` serves the cache **without rescanning** (background refresh on staleness) |
| `server/__tests__/LocalLlmRuntimeService.test.ts` | 14 | **Model-Fabric (rewritten for Phase 8 hot-swap).** Binary-absent → unavailable; binary present + no model → **available-but-idle** (hosts on demand); **boot-resumes the persisted last model** on `/health` 200 (else warms nothing); `ensureModelLoaded` **hot-swap** (stops current, spawns requested) + **no-op when already loaded**; `_computeGpuLayers` VRAM fit (explicit override honored; auto fits→999 / partial-offload for oversize / CPU-0 with no GPU); stop SIGTERM; auto-restart w/ backoff; **max-restarts ceiling** (respawns now serialized through the lifecycle queue) |
| `server/__tests__/AiProviderService.streamChat.test.ts` | 25 | **Model-Fabric.** `chatLocalLlm` (now hot-swaps via `ensureModelLoaded(target)`) + native tool-call cases: OpenAI `tool_calls` fragment assembly; Anthropic `tool_use` block assembly; Ollama streaming native tool_calls; llama-server grammar-request shape; llama-server grammar-rejection fallback; Valet-disagrees-with-explicit-provider non-redirect; Valet-throws doesn't block turn; pinned-peer-not-discoverable throws; pinned-peer-network-failure-mid-turn throws |
| `server/__tests__/toolSchemas.test.ts` | new | **Model-Fabric.** `toOpenAiToolSchemas()`, `openAiToolsToAnthropic()`, `buildLocalLlmToolGrammarSchema()`, `buildLocalLlmToolReminder()` — single source of truth for 3 built-in agent tools |
| `server/__tests__/ModelCatalogService.test.ts` | 24 | **Model-Fabric (+Phase 8).** Aggregation, dedup, mesh-peer merge, cloud key-gating, partial-failure isolation; `collectLocalOnly` mesh/cloud exclusion; `sizeMb`; **Sovereign gate** (cloud source never called when sovereign); mesh-peer `nativeTools:false`. **Phase 8:** runtime lists **all** indexed GGUFs as `omnecor-runtime`; **`loaded` flag only on the ready model** (not mid-swap/failed); **Ollama API source skipped when the runtime is available** (index covers the store — no double-listing), included only when unavailable |
| `server/__tests__/AiProviderService.selectPeerNode.test.ts` | 4 | **Model-Fabric.** Pin-to-exact-peer / pin-to-self / pin-not-found / no-pin-unchanged; `meshNode` mocked |
| `server/__tests__/blueprintCalcEngine.test.ts` | 37 | **Session-39/40 (Blueprint Studio).** Golden-value tests vs. hand-computed textbook formulas: conversions; section props (rect/rect-tube/round-bar/round-tube, impossible-wall reject); `beamAnalysis` (PL/4 + PL³/48EI, UDL 5wL⁴/384EI, cantilever PL + PL³/3EI, overload → critical + SF<1, **FDM knockdown**); `columnBuckling` (Euler + K-factor); carpentry geometry (rafter/stairs/miter/triangle); `fastenerGroupCheck`; 1D/2D/fabric nesting (kerf, oversized→unplaced, no-overlap AABB); mass/board-feet + SF policy. **Session-40 — the six added calcs:** `filletWeld` (0.707·leg throat + 0.6·FEXX capacity + overload critical), `boltedConnection` (governing of bolt-shear/bearing/tear-out + edge-too-close → tear-out≈0), `torsion` (solid τ=T·c/J & θ=T·L/GJ, tube J=π(od⁴−id⁴)/32), `woodJoinery` (NDS W=1800·G^1.5·D^0.75 + end-grain refuse), `printedPart` (effective walls+infill area, ×0.5 across-layers knockdown), `heatCheck` (PLA fails hot-car/sun, PETG passes sun with margin). **`/review` guards:** unknown `heat_check` scenario / `printed_part` mode / strength basis each throw a clear error (not a crash / silent bending / silent always-fail) |
| `server/__tests__/blueprintCadService.test.ts` | 23 | **Session-39/40 (Blueprint Studio).** JSCAD in-process engine (exact volume/bounds/mass, named multi-part, booleans, `console.log` capture, no-`main()`/no-solid reject, **sandbox denies `require`/`process`**), OpenSCAD status-probe; `meshUtils` STL round-trip (binary + ASCII), feature-edge extraction (12 cube edges) + orthographic projection; three-view SVG (real dims + title block, imperial ft/in) + minimal DXF; pattern gen (seam offset, 1:1 multi-page PDF, empty-set reject); **`ChatAgentRunner` extraTools prompt integration** (built-in wording unchanged; injected tools from JSON Schema; built-ins omitted for domain-only). **Session-40 — DXF reader** (`parseDxf2d`/`outlineSvg`): LINE + LWPOLYLINE → segments with correct bounds, open vs. closed polyline, outline-preview SVG, empty-DXF → no segments |

### 1.3 Client — Library Tests (15 files, ~230 tests)
...
| `client/src/lib/agentStream.test.ts` | 12 | **Chats-Agentic-Upgrade.** `applyAgentEvent` / `applyJobCompletion` pure reducer: fold immutability, approval transitions, job correlation |
| `client/src/lib/chatBlocks.test.ts` | 8 | **Chats-Agentic-Upgrade.** `AssistantBlock` union, `FileDiff`, `isToolBlock`, `blockDotIntent`, `flattenBlocksToText`, `TOOL_FAILURE_STATUSES` helpers |
| `client/src/lib/app.store.test.ts` (messageQueue slice) | 11 | **Chats-Agentic-Upgrade.** FIFO queue: enqueue/dequeue/pop-last/clear/remove-by-index |
| `client/src/__tests__/describeCatalogHost.test.ts` | 8 | **Model-Fabric.** `describeCatalogHost()` brand derivation: local omnecor-runtime, local ollama, mesh-peer llamacpp (→ Omnecor brand), mesh-peer ollama, cloud, phone |
| `packaging/android/omnecor-hq/lib/_core/ai-models.test.ts` | 8 | **Model-Fabric.** APK `fetchCatalog`/`listCatalogGroups` grouping, mesh-per-node separation, cloud collapse, phone merge, offline degradation |
...
## Section 2 — Backend Routers (51 routers in `server/routers/`)
...
### `blueprint` router (`blueprintRouter.ts`) — AI-assisted fabrication planning

| Procedure | Status | Tested By | Missing |
|---|---|---|---|
| `blueprint.list` / `create` / `get` / `update` / `delete` | ✅ VERIFIED | blueprintRouter.test.ts (CRUD, mapId filter, **per-user ownership isolation**, child-row cascade, path-stripped `get` snapshot) | — |
| `blueprint.listMessages` / `appendMessage` | ✅ VERIFIED | blueprintRouter.test.ts (ordered thread, `blocks` + `tokenCount`, ownership) | — |
| `blueprint.upsertBomItem` / `deleteBomItem` / `upsertCutItem` / `deleteCutItem` | ✅ VERIFIED | blueprintRouter.test.ts (upsert/update/delete, mm + miter-angle fidelity, ownership) | — |
| `blueprint.materials.categories` / `search` | ✅ VERIFIED | blueprintRouter.test.ts (categories + real-property search against the 61-entry catalog) | — |
| `blueprint.engineStatus` | ✅ VERIFIED | blueprintCadService.test.ts (`getEngineStatus` — JSCAD up, OpenSCAD probe never throws) | — |
| `blueprint.agentStream` (planning subscription) | 🤖 HARNESS-DRIVABLE | Domain toolset + persistence helpers fully covered (blueprintRouter.test.ts: `update_plan`/`set_bom`/`set_cut_list`/`engineering_calc`/`optimize_cuts`/`list_materials`, sovereign web-tool omission); reuses the tested `ChatAgentRunner`. **Live-verified end-to-end 2026-07-13** (real Gemini turn: catalog lookup → beam check SF 12.54 PASS → BOM/cut → CAD compile → PDF). | Automated subscription-level stream test (same `createContext`/singleton constraint as other stream routers — see §12) |
| `blueprint.getFile` / `deleteFile` | 🔬 PARTIAL | blueprintRouter.test.ts (delete ownership); `readArtifact` path-escape guard covered indirectly | Direct `getFile` base64 round-trip |
| `blueprint.exportPdf` | ✅ VERIFIED | `buildPlanPdf` booklet structure + WinAnsi transliteration (blueprintPlanPdf.test.ts, Session-40) | — |
| `blueprint.exportBom` | ✅ VERIFIED | blueprintRouter.test.ts (CSV RFC-4180 escaping, supplier grouping, known-price total, ownership) | — |
| `blueprint.importGeometry` | ✅ VERIFIED | blueprintRouter.test.ts (STL→full part mesh/stl/drawings + `imported` flag; DXF→outline drawing; empty-DXF + non-owner reject) + DXF reader unit tests (blueprintCadService.test.ts) | — |
| `blueprint.generateConcept` | 🤝 HARDWARE / 🌐 | Sovereign image-provider gate + tool omission tested (blueprintRouter.test.ts) | Real render needs local ComfyUI or cloud fal/openart |

**Engines behind the router (unit-tested separately):** `calcEngine.ts` (37 golden-value tests, blueprintCalcEngine.test.ts — 13 structural/geometry types incl. weld/bolt/torsion/joinery/printed/heat) · `BlueprintCadService`/JSCAD + `meshUtils`/`drawingSvg`/`patternPdf`/`geometryImport` (23 tests, blueprintCadService.test.ts) · `bomExport`/`fileStore` (revision lineage) covered via the router. FEA (`fea_bridge.py` + `BlueprintFeaService`) is an optional Python dep — availability-probed, degrades gracefully, and now runs **decoupled from the chat stream** (survives disconnect; running→completed verified with a mocked service); a live Gmsh solve is 🤝 HARDWARE.
...
## Section 5 — Frontend Web Pages (16 routes)
...
| Feature | Status | Notes |
|---|---|---|
| Plan CRUD + plans rail (Neural-Map-scoped) | ✅ VERIFIED | blueprintRouter.test.ts (create/list/get/update/delete, mapId filter, ownership) |
| Agentic planning conversation (`AssistantStream` + `ModelSelector`) | 🤖 HARNESS-DRIVABLE | Reuses the tested agentic stream renderer; toolset + persistence covered; **live-verified end-to-end 2026-07-13** (real Gemini turn wrote BOM/cut/calcs/CAD) |
| BOM + Cut List tabs (hand-editable) | ✅ VERIFIED | blueprintRouter.test.ts (upsert/delete + mm/angle fidelity) |
| Drawings tab (inline three-view SVG) | ✅ VERIFIED | blueprintCadService.test.ts (`buildDrawingSvg` real dims + title block) |
| 3D tab (compiled parts + FEA heatmap overlay) | 🌐 MANUAL REQUIRED | WebGL viewer; JSCAD compile/mesh math unit-tested |
| Patterns tab (true-scale PDF) | ✅ VERIFIED | blueprintCadService.test.ts (`buildPatternPdf` 1:1 multi-page) |
| Simulation tab (calc + FEA results) | ✅ VERIFIED | blueprintCalcEngine.test.ts (35 golden values incl. weld/bolt/torsion/joinery/printed/heat); FEA shows a RUNNING badge → completed (async); live Gmsh solve is 🤝 HARDWARE |
| Export PDF booklet | ✅ VERIFIED | blueprintPlanPdf.test.ts (structure + WinAnsi transliteration) |
| Shopping export (BOM CSV + buy-list) | ✅ VERIFIED | blueprintRouter.test.ts `exportBom`; "Export list" button downloads CSV |
| Geometry import (STL / DXF) | ✅ VERIFIED | blueprintRouter.test.ts `importGeometry` (STL→full part, DXF→outline); "Import STL/DXF" control on 3D tab |
| Part revision history (Files tab versions) | ✅ VERIFIED | blueprintRouter.test.ts revision lineage; superseded rows dimmed, latest read by Drawings/3D/PDF |
| Concept render button | 🤝 HARDWARE / 🌐 | Needs local ComfyUI or cloud fal/openart; sovereign gate tested |
...
| `(tabs)/ai-node.tsx` — AI Node (LiteRT-LM) | ✅ VERIFIED | live on S25 Ultra (2026-07-04) | `.litertlm` load + on-device inference confirmed: DeepSeek-R1-1.5B **and Gemma-4-E4B** answered prompts, PC offline; Gemma-4 on the **GPU** backend, no crash. Loader hardened (single-engine/single-load) — see Session-32 note |
...
- [x] App launches without crash (Metro bundle is standalone — no dev server) — verified 2026-07-04
- [x] LiteRT-LM model download flow (`.litertlm` file) — catalog download + SAF folder-scan + import all verified 2026-07-04
...
- [x] **Long-running stability** — `pnpm dev` left running unattended overnight for **8+ hours** (user, 2026-07-06 → 2026-07-07): server remained up, fully responsive, no crash, no errors on return — **verified 2026-07-07**
...
- [x] **LiteRT-LM model** — Download `.litertlm` model, run on-device inference, verify response — **verified 2026-07-04**: DeepSeek-R1-1.5B + Gemma-4-E4B loaded and answered on-device (PC offline), Gemma-4 on GPU, no crash
...
- **~~Standalone prod bundle can't resolve the native libSQL binding.~~ ✅ RESOLVED (2026-07-15).** `node dist/index.js` threw `Cannot find module '@libsql/linux-x64-gnu'` because libsql loads its `.node` addon via a dynamic `require(`@libsql/${target}`)` that esbuild leaves as a runtime require resolved next to `dist/`, and nothing copied the binding there. Fixed in `scripts/build-server.mjs`: after the esbuild step it copies every installed `@libsql/<platform>` package that ships an `index.node` into `dist/node_modules/@libsql/`, making `dist/` self-sufficient (no more `NODE_PATH` workaround). Verified: `require.resolve` lands in `dist/node_modules`, and the production bundle boots to `/health` `{db:true, migrationOk:true}` against an isolated temp data dir.
- **~~Ollama URL is read from two different settings sources.~~ ✅ RESOLVED (2026-07-15).** New `server/core_services/services/ollamaUrl.ts` `resolveOllamaUrl()` is the single canonical resolver (via **SettingsService**, chain `input → OLLAMA_BASE_URL → legacy ollamaUrl → ENV`). Now used by `AiProviderService.getOllamaUrl` (inference) **and** all three `systemRouter` reads — `aiProviders` (status), `detectHardware`, `checkDependencies` (the latter two previously read a raw file / only `ENV`). A URL saved under the legacy `ollamaUrl` key (the exact live-debugged mismatch) now resolves identically for status and inference. +4 regression tests (`server/core_services/services/__tests__/ollamaUrl.test.ts`). Ties to [[settings-architecture]].
