...
async function publishOne(
  ...
  const pubAccount: PublishAccount = {
    ...
    // Decrypt at the boundary so PublishingService stays crypto-agnostic and
    // works with the raw credential (Bearer token / bot token / webhook URL).
    oauthToken: decryptPlatformToken(account.oauthToken),
    oauthRefreshToken: decryptPlatformToken(account.oauthRefreshToken) || null,
  ...
  try {
    ...
    if (result.refreshedToken) {
      await db
        ...
        .set({
          oauthToken: encryptPlatformToken(result.refreshedToken.accessToken),
          ...(result.refreshedToken.refreshToken ? { oauthRefreshToken: encryptPlatformToken(result.refreshedToken.refreshToken) } : {}),
