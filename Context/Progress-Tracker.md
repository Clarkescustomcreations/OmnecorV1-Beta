...
## 🚦 Current Status
*   **Active Phase:** Phase 5: Mobile App Realization & Verification (1/5 — F23 ✅)
*   **Next Task:** Feature 24: Mobile 3D Canvas Interactivity
...
- [~] **Feature 23b: Always-Listening Voice Mode (added — wake-word → on-device STT → PC persona)**
  ...
  *   *Deferred — needs Android build machine:* native typed **Foreground Service** (`foregroundServiceType="microphone"` + persistent notification) to hold the mic with the app closed, and a background VAD `setCaptureProvider`. Irreducibly native Kotlin — not shipped as an unverified stub (AGENTS.md). Also pending on the build machine: train the "Hey Omnecor" `.ppn` (Picovoice console) + `setKeywordPath`; `pnpm prebuild:android` → `pnpm apk:release`; on-device test. **Until then the feature works only while the app is foregrounded** (wake word + Test both run the full loop).
