...
### Agentic sub-agent harness — bugs found + fixed during the local-model benchmark (2026-07-16)

Building `server/scripts/benchmark-agentic.ts` (the empowered-vs-bare capability benchmark that replaced the doc's old "equals a 70B" hand-wave) surfaced **two real, shipped harness bugs** in `LocalSubAgentWorker` — both silently caused the background sub-agent to execute **zero** tool calls (it mistook an un-parsed tool call for a final answer). Both are now fixed; recording so they don't regress.

- **`LocalSubAgentWorker` only recognized `<tool_call>` tags or ```` ```json ```` fences — small models emit BARE JSON.** qwen2.5-coder:7b (and many 3–7B models) return a tool call as a bare object in the content — `{"name":"execute_sandbox","arguments":{…}}` (or `{"action":…}`) — with no tag, no fence, and it is **not** a native `message.tool_calls`. The old regex-only parser missed it entirely → 0 tool steps → the bare JSON was returned as the "answer." **Fix:** added a balanced-brace `extractFirstJsonObject()` fallback that also accepts an inline bare object, normalizing all three shapes to `{action, …params}` (and only surfacing a parse error as retryable when a `<tool_call>` tag was explicitly opened, so a non-tool JSON answer isn't misread). **Lesson:** for local-model tool loops, accept every framing (tagged / fenced / bare) and normalize — never assume one convention.
- **`chat()` streams, so the Ollama streaming path dropped native `tool_calls` for the sub-agent.** `AiProviderService.chatOllama` had a shim that converts native `tool_calls` → `<tool_call>` text — but only in the **non-streaming** branch (comment even said "for LocalSubAgentWorker"). The worker calls `chat()`, which passes an `onChunk` → the **streaming** branch, where `tool_calls` were parsed into a structured `toolCalls` field that `chat()` (string-join of content only) discards. So a native-tool-calling model's calls vanished. **Fix:** shim native `tool_calls` into the streamed **content** in the streaming branch too, scoped to `routingMode === "sub_agent_internal"` (leaves the interactive chat's native path untouched). **Lesson:** a shim in the non-streaming branch is dead code for any caller that goes through `chat()`/streaming.
- **`LocalSubAgentWorker` had no total-step ceiling — only a failure budget.** `retries` is incremented on parse/tool/harness *errors* only; a model making endless *successful* tool calls without ever finishing would loop unbounded. **Fix:** added `maxSteps` (default 50) as a hard iteration cap independent of `maxRetries`, plus an opt-in `onEvent` trajectory observer (used by the benchmark to count real tool steps).
- **`server/scripts/benchmark-qwen-coder.ts` crashed on run** — it hard-read `agents/4B-Logic-Controller-Prompt.md`, which does not exist in the repo (`agents/` is empty). So the doc pointed at "benchmark" evidence that could never execute. **Fix:** the missing prompt is now optional (falls back to the worker's built-in tool prompt).
- **`benchmark-agentic.ts` hung forever after printing results (no `process.exit`).** The script imports service singletons — the libSQL DB pool, the OMMESH `MeshNode`, `LocalLlmRuntimeService`, the embedding engine — that hold open handles/timers, so Node's event loop never drains and the process sits in `Sl` sleep indefinitely after `main()` resolves. Every run "completed" (full aggregate in the output file) but never exited, so orphaned processes piled up and competed for the model. **Fix:** `main().then(() => process.exit(0)).catch(() => process.exit(1))` — a long-lived script that pulls in server singletons MUST exit explicitly. **Lesson for any `server/scripts/*` one-shot:** import a service singleton → you own the exit.
### TD-061: Background sub-agent (`LocalSubAgentWorker`) couldn't run multi-line scripts; tool-fail recovery stalled (2026-07-16)

- **File**: `server/core_services/services/LocalSubAgentWorker.ts`. **Found by** the benchmark's "hard" tier (algorithmic tasks needing real code, not a stdlib one-liner).
- **Gap 1 — no multi-line script path.** The only sandbox tool was `execute_sandbox {command, args}` run via `execFile`. A small model writing a real program (`def`/`while`/`for`) put it into `python3 -c "<arg>"`, where the body collapses onto one physical line → `SyntaxError`. So genuinely algorithmic tasks scored **0/5** even though the model's *logic* was correct — a tooling gap, not a reasoning gap. (Production chat has `edit_file`+`run_command` to write a script and run it; the background worker had neither.) **Fix:** `execute_sandbox` now accepts `code_lines` (array, one source line per element — robust for small models, no newline-escaping in JSON) or `code` (string); it writes to a temp file and runs `python3/node <file>`. Plus a safety net that reroutes a multi-line `-c` to a file, and prompt guidance teaching `code_lines`. Empowered climbed 0/5 → 20%+ (Collatz solved).
- **Gap 2 — tool-fail recovery couldn't converge.** Try-Fail-Fix *fired* (errors fed back, up to `maxRetries`) but stalled: (a) the raw `SyntaxError` didn't diagnose the cause, and (b) nothing stopped the model re-sending the **identical** failing call, so it burned all 8 retries on the same wall. **Fix (recovery additions):** targeted feedback now (i) detects flattened multi-line code and says "resend as `code_lines`" with the exact shape, (ii) detects a **timeout** and says "use a faster algorithm — brute force is too slow", and (iii) a **loop-breaker** detects an identical repeat call and tells the model to change approach.
- **Honest residual (model ceiling, not a bug).** Even with both fixes, a 7B still (a) sometimes writes syntactically-invalid one-line python instead of using `code_lines`, (b) writes buggy logic (wrong comprehension → `TypeError`), or (c) breaks the tool-call JSON with code content (`no_tool_use`). So the hard tier stays well under 100% — the correct, discriminating result: empowerment is transformative on **offload-able** tasks (deterministic compute → 100%) but bounded by the model's own **code-writing + tool-format discipline** on tasks where the model must author the algorithm. That gap is the target for future empowerment work (better tool-call formatting guidance, a structured `write_file`+`run` pair, or a stronger coder model).
- **Risk**: Medium (the multi-line gap silently capped the background agent on any real coding task). **Status**: Gaps 1 & 2 fixed; residual is a measured model-capability ceiling, tracked for improvement.

- **Operational gotcha — `pkill -f benchmark-agentic` inside a launch command kills its own shell wrapper.** The harness runs each Bash command via a wrapper whose *cmdline contains the command string* (`… npx tsx server/scripts/benchmark-agentic.ts …`). A `pkill -9 -f benchmark-agentic` at the start of that same command matches — and kills — that wrapper before `tsx` ever starts, so the command dies with a non-descriptive exit 1 and an empty output file. Kill lingering processes in a **separate** prior command (or match a narrower pattern that excludes the wrapper, e.g. the node loader path), never inline with the launch.

### OMMESH / own-runtime reachability gotchas (2026-07-16)

- **Omnecor's own `llama-server` runtime is localhost-bound and self-managed — you cannot point a remote node's `llamacpp` path at a peer's own-runtime.** `LocalLlmRuntimeService.getBaseUrl()` is hardcoded to `http://127.0.0.1:${port}` (e.g. `.201` serves on `127.0.0.1:8014`), and the service *spawns/owns* its child process. Consequences for the "weak laptop + secondary AI-server over OMMESH" topology: (1) the laptop's `llamacpp` provider can't reach the server node's own-runtime over LAN; (2) running the agentic benchmark as a *second* process on the server node conflicts on the runtime port; (3) `ommesh.routeInference` (mTLS `:3001`) is **single-shot**, not wired into the agentic `chat()` tool loop. So today the only LAN-reachable inference on a peer is its **Ollama** (`:11434`), or you run the tool loop **co-located** with the own-runtime. A true "laptop tool-loop → peer own-runtime inference" path would need either the own-runtime LAN-exposed (bind + auth) or the mesh offload integrated into `chat()`.
- **`/health` `localLlm:false` right after boot is a race, not a failure** — the health check can fire at ~5 s uptime while the own-runtime is still loading the GGUF (model "Ready" landed at ~8 s in the `.201` log). Re-check after the model-loaded line before concluding the runtime is down.
- **Ollama is chat-focused and unreliable for tool calls** — with `tools` supplied, qwen2.5-coder:7b via Ollama variously returns native `tool_calls`, or (more often) a bare JSON object in the content; neither reached the sub-agent before the two fixes above. This is concrete motivation for LOCAL_MODEL_EMPOWERMENT §1 (Omnecor owning its own runtime + a format-agnostic tool parser) rather than depending on Ollama's tool-calling.

### TD-060: Generalist brain charter REGRESSES small-model tool use on simple tasks (2026-07-16)

- **Files**: `brains/generalist.obp` (charter), `server/_core/brainContext.ts` (`injectBrainContext`), `server/core_services/services/LocalSubAgentWorker.ts` (prompt assembly).
- **Found by**: the 12-run agentic benchmark (`benchmark-agentic.ts`). Attaching the Generalist brain **dropped** the empowered qwen2.5-coder:7b from **72/72 (100%)** to **71/72 (98.6%)** on pure-computation tasks, and left one extra long-horizon run "unverified". A brain should help or be neutral — **never** drop tools-only performance. Real regression.
- **Mechanism**: `injectBrainContext` prepends the Generalist charter (**~2892-char charter → a ~7431-char / ~1858-token** injected system block) *ahead of* the worker's tool instructions. That charter is heavy **agentic discipline** written for complex multi-step coding work, and its top rules actively conflict with fast autonomous single-shot tool use: *"Plan before acting … present the plan. **Confirm consequential or ambiguous decisions with the user BEFORE acting**"* and *"**Track the work in TODO.md.** Create it from your plan …"*. On a one-line "compute X with python3" task a 7B model tends to follow the charter — emit a plan / hedge / try to make a TODO — instead of immediately firing the tool call, so it occasionally derails. There is **no user** in a headless sub-agent loop, so "confirm with the user before acting" is not just noise, it's counter-productive.
- **Risk**: Medium — silent accuracy/step regression whenever a brain is attached to a small model doing tool-driven work (the main empowerment path). Grows with charter size.
- **Status**: **RESOLVED 2026-07-16/17.** (1) The Generalist charter is now **proportional** — "a simple, single-step, reversible task is done directly, no planning ritual / no TODO / no asking permission; reserve the discipline for multi-step work." Verified at 12-run: brain-arm long-horizon steps went from mean 14.8 / **max 40 (ceiling spiral)** / 10-of-12 verified → mean 12.6 / max 14 / **12-of-12**, and Part B +brain 71→72/72. (2) **Deeper finding + fix — the "blueprint" refactor.** The hard-tier A/B proved the *reasoning process*, not domain facts, is what lifts a small model: Generalist **+20 pts** on hard tasks vs the Coding brain's **+0 (draw)** despite being the domain expert. So the proportional reasoning discipline was extracted to `brains/sources/_reasoning-base.ts` (`REASONING_BASE`) and **every expert charter is now `REASONING_BASE` + a domain layer** (all 8 experts refactored + rebuilt). Grafting the base onto the Coding brain flipped it from a draw to **+10 pts** — validated. Authoring standard captured in `docs/ai-agents/MAKING_EXPERT_BRAINS.md`. (Small benchmark samples → directional, but consistent and charter-analysis-backed.)
