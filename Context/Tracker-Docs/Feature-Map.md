...
- **Background Jobs**: `FileSystemWatcherService` (monitors indexed folders for graph updates), `MemoryArchitectService` (ChromaDB semantic index of the mapped file tree)
- **Dependencies**: `react-flow-renderer` / `@xyflow/react` (canvas), `fileTreeToNetwork.ts` (file→graph converter), ChromaDB (long-term vector index), `react-force-graph` (Force/Hierarchical/Mind-Map/Circular layouts)
- **Settings**: Settings → Knowledge tab (folder path, file filters, auto-index toggle); Visual Controller sidebar (layout engine, node size 20–50 px, animation speed, GPU acceleration, auto-clustering, show labels); `omnecor_visual_control_sync` BroadcastChannel syncs visual prefs to external pop-out
- **API**: `trpc.neuralMaps.list`, `trpc.neuralMaps.create`, `trpc.neuralMaps.update`, `trpc.neuralMaps.delete`, `trpc.neuralMaps.get`
...
- **Background Jobs**: `MemoryArchitectService` (auto-summarization after 50 messages — keeps last 6 + system summary), `HashTrackerService` (action-hash loop detection, 3-rep threshold → HITL alert), `AiProviderService` (spend tracking per token)
...
- **Router**: `server/phase2/routers/agentRouter.ts` → `trpc.agent.*`; `server/routers/pipelineRouter.ts` → `trpc.pipeline.*`; `server/routers/hitlRouter.ts` → `trpc.hitl.*`; `server/routers/workflowRouter.ts` → `trpc.workflow.*`
