...
*   **Next:** install the Windows `.exe`; sideload `app-release.apk`; run the Linux AppImage — set the same `OMMESH_SECRET` on all three + same LAN → verify mesh discovery/routing live (cross-node inference now wired end-to-end). Then F23b on-device wake-word test; optional social live-test.
