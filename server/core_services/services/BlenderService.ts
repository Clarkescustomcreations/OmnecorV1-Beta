...
export class BlenderBridge extends EventEmitter {
  ...
  async exportFile(blendFile: string, outputPath: string): Promise<string> {
    ...
    const safeBlend = JSON.stringify(blendFile);
    const getPrint = (msg: string) => `print(${JSON.stringify(JSON.stringify({ status: "success", message: msg, output: outputPath }))})`;

    // The generated expression runs in a *fresh* headless Blender (executeExpression
    // → executeScript with no blendFile), so we MUST open the source .blend inside
    // the script. Without this, every export silently produced Blender's default
    // startup cube instead of the requested model. Only .blend files can be opened;
    // for any other source we export whatever the startup scene contains.
    const openStep =
      path.extname(blendFile).toLowerCase() === ".blend"
        ? `bpy.ops.wm.open_mainfile(filepath=${safeBlend})\n`
        : "";
    ...
    const exportScripts: Record<string, string> = {
      ".glb": `
import bpy
${openStep}bpy.ops.export_scene.gltf(filepath=${safeOutput}, export_format='GLB')
${getPrint("Exported to GLB")}
`,
      ".fbx": `
import bpy
${openStep}bpy.ops.export_scene.fbx(filepath=${safeOutput})
${getPrint("Exported to FBX")}
`,
      ".obj": `
import bpy
${openStep}bpy.ops.wm.obj_export(filepath=${safeOutput})
${getPrint("Exported to OBJ")}
`,
      ".stl": `
import bpy
${openStep}bpy.ops.export_mesh.stl(filepath=${safeOutput})
