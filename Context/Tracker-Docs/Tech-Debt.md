...
- **`ZERO_LOGIN_MODE` runs as `scrapper`, NOT sovereign** (`context.ts` creates the local-zero-login user with `executionMode:"scrapper"`, `role:"admin"`). So `cloudProcedure`s (cloud AI, integrations) **work** under zero-login — cloud is allowed, only spend-tracked.
