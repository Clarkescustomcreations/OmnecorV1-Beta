...
/**
 * Baseline YARA ruleset written to disk on first run.
 *
 * This is intentionally a compact, dependency-free starter set covering the
 * highest-signal indicators for files arriving through user uploads:
 *   - Native executable headers (PE / ELF / Mach-O) hidden in non-binary
 *     contexts or archives.
 *   - Interpreter shebangs and PowerShell/cmd download-and-execute payloads.
 *   - Common PHP/JSP/ASP web shells and `eval`-based obfuscation.
 *   - The EICAR test string, retained so the pipeline can be verified
 *     end-to-end with a known-benign sample.
 *
 * It is NOT a substitute for a maintained commercial/community feed. Operators
 * should layer in full rule sets (Florian Roth's signature-base, the
 * Yara-Rules project, VirusTotal community rules) by appending to or replacing
 * the generated rules.yar file; `scanWithYara` consumes whatever is on disk.
 *
 * Note: this string is a JS template literal, so every backslash destined for
 * YARA is doubled and `$` that should reach YARA is escaped as needed.
 */
const DEFAULT_YARA_RULES = String.raw`
/*
 * Omnecor baseline malware indicators.
 * Generated by SecurityService.initialize(). Safe to edit/extend.
 */

rule Omnecor_EICAR_Test_File
{
    meta:
        description = "EICAR antivirus test file (benign) - verifies scanning pipeline"
        severity = "info"
    strings:
...
rule Omnecor_PE_Executable
{
    meta:
        description = "Windows PE/MS-DOS executable header"
        severity = "high"
    strings:
        $mz = { 4D 5A }
        $pe = { 50 45 00 00 }
...
rule Omnecor_ELF_Executable
{
    meta:
        description = "Linux/Unix ELF executable or shared object"
        severity = "high"
    strings:
        $elf = { 7F 45 4C 46 }
    condition:
...
rule Omnecor_MachO_Executable
{
    meta:
        description = "macOS Mach-O executable (32/64-bit, LE/BE, fat binary)"
        severity = "high"
    strings:
        $macho_le64 = { CF FA ED FE }
        $macho_le32 = { CE FA ED FE }
        $macho_be64 = { FE ED FA CF }
        $macho_be32 = { FE ED FA CE }
        $macho_fat  = { CA FE BA BE }
    condition:
        ($macho_le64 at 0) or ($macho_le32 at 0) or
        ($macho_be64 at 0) or ($macho_be32 at 0) or
        ($macho_fat  at 0)
}

rule Omnecor_Script_Shebang
{
    meta:
        description = "Script with shell/interpreter shebang invoking a shell"
        severity = "medium"
    strings:
        $sh   = "#!/bin/sh"
        $bash = "#!/bin/bash"
        ...
        $py   = "#!/usr/bin/python"
    condition:
        ($sh at 0) or ($bash at 0) or ($env at 0) or
        ($perl at 0) or ($py at 0)
}

rule Omnecor_Suspicious_PowerShell
{
    meta:
        description = "PowerShell download/execute or encoded-command payload"
        severity = "high"
    strings:
        $a = "powershell" nocase
        $b = "-enc" nocase
        ...
        $h = "FromBase64String" nocase
        $i = "-w hidden" nocase
        $j = "-nop" nocase
    condition:
        $a and (2 of ($b, $c, $d, $e, $f, $g, $h, $i, $j))
}

rule Omnecor_Shell_Download_Execute
{
    meta:
        description = "Curl/wget piped directly into a shell interpreter"
        severity = "high"
    strings:
        $curl_sh = /curl\s+[^\n]{0,200}\|\s*(ba)?sh/ nocase
        $wget_sh = /wget\s+[^\n]{0,200}\|\s*(ba)?sh/ nocase
...
rule Omnecor_Web_Shell
{
    meta:
        description = "Common PHP/JSP/ASP web shell and dynamic eval patterns"
        severity = "critical"
    strings:
        $php_open  = "<?php"
        $php_eval1 = /eval\s*\(\s*\$_(POST|GET|REQUEST|COOKIE)/ nocase
        $php_assert = /assert\s*\(\s*\$_(POST|GET|REQUEST)/ nocase
        $php_sys   = /(system|shell_exec|passthru|popen|proc_open)\s*\(\s*\$_(POST|GET|REQUEST)/ nocase
        $php_b64   = /base64_decode\s*\(\s*\$_(POST|GET|REQUEST)/ nocase
        $jsp_rt    = "Runtime.getRuntime().exec"
        $asp_eval  = /eval\s*\(\s*Request/ nocase
    condition:
        ($php_open and ($php_eval1 or $php_assert or $php_sys or $php_b64))
        or $jsp_rt or $asp_eval
}

rule Omnecor_Obfuscation_Indicators
{
    meta:
        description = "High-entropy / obfuscation primitives often used to hide payloads"
        severity = "medium"
    strings:
        $py_exec   = /exec\s*\(\s*(base64|__import__|compile)/ nocase
        $py_b64    = "base64.b64decode" nocase
        $js_eval_atob = /eval\s*\(\s*atob\s*\(/ nocase
        $js_fromcc = "String.fromCharCode("
        $vbs_exec  = "WScript.Shell" nocase
    condition:
        2 of them
}
`;
...
export class SecurityService {
  ...
  async initialize(): Promise<void> {
    ...
    // Provision the YARA ruleset if it does not already exist on disk.
    // These rules ship as a baseline so that uploaded files are screened
    // against real malware indicators (executable headers, scripting
    // payloads, web shells, obfuscation) instead of only the EICAR test
    // string. Operators may extend rules.yar with vendor/community rule
    // sets (e.g. Florian Roth's signature-base, Yara-Rules project) without
    // changing this code — the scanner loads whatever is present at the path.
    try {
      await fs.access(this.yaraRulesPath);
    } catch {
      await fs.writeFile(this.yaraRulesPath, DEFAULT_YARA_RULES, { mode: 0o600 });
    }
  }
