...
export interface AsyncJobContext {
  ...
  label?: string;
  /**
   * Whether finishing this job should re-prompt the AI with its result. True
   * (default) for AI-launched `start_job` background work — the whole point of
   * the async-job flow. False for user-initiated code Runs (the "▶ Run" button):
   * the run is a test whose result belongs in its own job box, and must not
   * silently kick the model into regenerating code the user didn't ask it to.
   */
