...
import subprocess
import sys
# Force Unsloth to return materialized logit tensors instead of its lazy
# logits-as-a-function memory optimization. Recent TRL computes per-token
# entropy via `outputs.logits.shape` inside compute_loss, which crashes on the
# lazy form ("'function' object is not subscriptable"). Must be set BEFORE the
# unsloth import. Costs a little VRAM; correctness over memory here.
...
def _bf16_supported() -> bool:
    """bf16 capability across Unsloth versions.

    `FastLanguageModel.is_bfloat16_supported()` was removed in newer Unsloth
    releases (moved to a top-level `unsloth.is_bfloat16_supported`), so resolve
    it defensively and fall back to torch's own capability probe.
    """
    try:
        ...
        pass
    try:
        probe = getattr(FastLanguageModel, "is_bfloat16_supported", None)
        if callable(probe):
            return bool(probe())
    except Exception:
        pass
...
def main():
    ...
    # TRL moved `dataset_text_field`/`max_seq_length`/`tokenizer` out of the
    # SFTTrainer constructor (into SFTConfig / `processing_class`) — build the
    # kwargs against whichever signature is installed. "text" is also modern
    # TRL's default text field, so omitting it is behavior-preserving there.
    import inspect
    ...
    _trainer_kwargs = {
        ...
        "args": training_args,
        "callbacks": [JsonLoggingCallback()],  # strict JSON progress for the Node parent
    }
    if "dataset_text_field" in _sft_params:  # legacy TRL
        _trainer_kwargs["dataset_text_field"] = "text"
        _trainer_kwargs["max_seq_length"] = max_seq_length
        _trainer_kwargs["tokenizer"] = tokenizer
    elif "processing_class" in _sft_params:  # modern TRL
    ...
    try:
        ...
        # Full traceback to stderr — the parent captures stderr into the job
        # record, and a bare str(e) is undebuggable for library-internal errors.
        import traceback
