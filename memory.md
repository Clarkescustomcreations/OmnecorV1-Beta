# Memory — AIAssistantPanel Polish, ComponentLibraryPanel Collapse & Prior Session Work

Last updated: 2026-06-21

## Important Note for Future AI Sessions
**Never overwrite this file fully.** Always carry over important information, findings, and changes made during previous sessions.
**DO NOT REMOVE THIS NOTE: Important: Read AGENTS.md Before Beginning The Next Session**


## What was built

- **AIAssistantPanel UI Polish:** Refactored `AIAssistantPanel.tsx` to use design system semantic tokens (`bg-card`, `text-foreground`, etc.) instead of raw hardcoded Tailwind colors, fixing the glaring white UI bug in dark mode.
- **Floating AI Toggle Fix:** Wired `EnhancedPCBEditor` to bubble up its `showAI` state to `3DDesigner` via an `onAIToggle` callback prop, ensuring the floating 'Ask AI About This View' button automatically hides when the AI Panel opens.
- **UI Registry Update:** Extracted visual patterns for `AIAssistantPanel` and appended them to `Context/UI-Registry.md`.
- **PCB/Schematic Toolbar:** Added a collapse button (`ChevronLeft`) to the `ComponentLibraryPanel` in the 3D Designer page (PCB/Schematic mode) so the component list can be hidden for more workspace.
- **UI Registry Update:** Extracted visual patterns for `ComponentLibraryPanel` and saved them to `Context/UI-Registry.md`.
- **Previous Session - UI Registry Update:** Adjusted the "Always Listening" section of `Context/UI-Registry.md` to specify "local NPU Whisper wake-word loop" and "whisper.rn native libs", removing stale references to Porcupine.

## Decisions made

- **State Bubbling:** Chose to pass an `onAIToggle` prop from `3DDesigner.tsx` to `EnhancedPCBEditor.tsx` instead of adding a global CSS class, keeping React state declarative and predictable.
- **AIAssistantPanel styling:** Converted user chat bubbles to `bg-accent` and assistant bubbles to `bg-muted` to seamlessly match the UI's color mode.
- **Collapsible Panel:** Modified `ComponentLibraryPanelProps` to accept an `onClose` function and wired it to `setShowLibrary(false)` in `EnhancedPCBEditor.tsx`.
- **Previous Session - Architecture:** Aligned the system architecture registry with the actual codebase implementation where Picovoice/Porcupine was replaced by a local custom sliding-window wake-word recording loop running via `whisper.rn` on the NPU.

## Problems solved

- Fixed an issue where the `AIAssistantPanel` loaded as glaring white in dark mode by replacing static color utility classes.
- Fixed a UI obstruction where the floating 'Ask AI About This View' button stayed visible on top of the newly opened AI Assistant panel.
- Created a more spacious workspace in the 3D Designer by allowing the Component Library panel to be dismissed directly from its header.
- **Previous Session:** Resolved stale references to Porcupine and Picovoice in `Context/UI-Registry.md`.

## Current state

- **Verification Status:** `pnpm check` typecheck passes with 0 errors.
- **Test Suite:** `pnpm test` runs and passes.
- **ComponentLibraryPanel:** Now includes a collapse button in its header.
...
## Past Sessions (Archived from Context)
