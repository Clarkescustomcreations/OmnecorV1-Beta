/**
 * Loading-quote pools + a shuffle-bag picker — the mobile port of
 * `client/src/components/chat/quoteBag.ts`. Kept React-free at module scope so
 * the no-repeat guarantee survives component remounts.
 *
 ...
 * quote is shown once before any repeat, and the last drawn quote is tracked
 * globally so no two consecutive draws ever match.
 *
 * Pools mirror the web set exactly (verified identical in the previous mobile
 * build) so web + APK show the same quotes.
 */
